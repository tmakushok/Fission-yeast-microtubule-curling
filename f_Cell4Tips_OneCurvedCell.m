%% This version of the function detects cell tips first until the point where
%% cell axes cross cell outline (going inwards along these axes).
%% Then it does the correction for the cell potentially being curved:
%% it finds the furthest point from cell center (from points being around
%% firstly detected pic). Cell center is not the one found automaticly, 
%% but the middle position on cell width line.

function [CellEnds, CellWidthEnds, CellsPixels, GoodCells] = f_Cell4Tips_OneCurvedCell(BigRegions, ImageFigNb, MinCellWidth, MaxCellWidth, MinCellLength, MaxCellLength)
%--------------------------------------------------------------------------
...
%% Adding of the cell having good geometry to the CellsPixels and GoodCells array        
    ...
    % Cell center is central position of cell width line
    x_CCenter = (CellWidthEnds(CWE_Lin, 1) + CellWidthEnds(CWE_Lin, 3)) / 2;
    y_CCenter = (CellWidthEnds(CWE_Lin, 2) + CellWidthEnds(CWE_Lin, 4)) / 2;
    GoodCells = [GoodCells; i, CellLength, CellWidth, Angle, x_CCenter, y_CCenter];
